 ...
 * Name: Rohan Mathur
 * NetID: rmathur2
 * This program optimizes circuit layouts by representing the circuit with cutlines through a binary tree, and then trying out a bunch of different
 * operations and seeing which has the best layout. It does this by a number of different operations, such as rotating, swapping modules, and swapping
 * the topology of nodes. Several other helper methods were also implemented to aid in this, such as determining whether nodes are leaf nodes or internal
 * nodes, and more. Also, a function to build the initial tree was implemented.
 */
...
int is_leaf_node(node_t* ptr) {
  // return true if it doesnt have either a left or right child, meaning it is a leaf node
...
int is_internal_node(node_t* ptr) {
  // return 1 if it has either a left or right child, meaning it is an internal node
  if(ptr->left != NULL || ptr->right != NULL)
    return 1;
  else
    return 0;
...
int is_in_subtree(node_t* a, node_t* b) {
  // check values of nodes, and return 1 if the current node is the one we are looking for
  if(a == b) {
    return 1;
  } else if(is_internal_node(a)) {
    // if its not the current node, recurse with the left and right children checking against B, and return the sum to check if the node is ever found
...
void rotate(node_t* ptr) {
  // perform a swap of W and H values for the current node
  // assign a temp value to hold the width value while it is overwritten
...
void recut(node_t* ptr) {
  ...
  // if the current cutline is V, change it to H
  if(ptr->cutline == V) {
    ptr->cutline = H;
  } else if(ptr->cutline == H) {
    // if the current cutline is H, change it to V
    ptr->cutline = V;
  }
...
void swap_module(node_t* a, node_t* b) {
  ...
  // swap the modules of the node
  // assign a temp variable to hold the a node's module first
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // assign the new module and cutline values
  if(ptr->module != NULL) {
    expression[*nth].module = ptr->module;
    expression[*nth].cutline = UNDEFINED_CUTLINE;
  }

  // if that was not just done, set module to null and copy over the pointers cutline
